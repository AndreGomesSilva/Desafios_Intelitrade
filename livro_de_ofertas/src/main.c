...
int get_notifications(int *number_notification, t_property *property)
{
  ...
  number = sscanf(buffer, "%d,%d,%lf,%d", &property->position, &property->action, &property->value, &property->quantity);
  if (number > 1 && number < 4 && validate_property(property))
    return TRUE;
  else
  {
...
int main(void)
{
  ...
  if (get_number_notifications(&number_notification))
  {
    for (int i = 0; i < number_notification; i++)
    {
      ...
      {
        init_actions(property, &book);
        printf("Operação concluída!\n");
        printf("Posicao: %d\nAção: %d\nValor: %lf\nQuantidade: %d\n", property->position, property->action, property->value, property->quantity);
        print_book(&book);
      }
    }
  }
